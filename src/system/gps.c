...
void GpsConvertPositionFromStringToNumerical( void )
{
    ...
    if( NmeaGpsData.NmeaLongitudePole[0] == 'W' )
    {
		...
		// printf("Latitude decimal = %lf\n", Latitude);
		// printf("Longitude decimal = %lf\n", Longitude);
}
...
void GpsPrintGpsData( int8_t *rxBuffer, int32_t rxBufferSize )
{
    printf("GpsDataSize: %d\n", rxBufferSize);
...
uint8_t GpsParseGpsData( int8_t *rxBuffer, int32_t rxBufferSize )
{
    ...
    if( GpsNmeaValidateChecksum( rxBuffer, rxBufferSize ) == false )
    {
		...
		// modified
		// GpsPrintGpsData( rxBuffer, rxBufferSize );
