...
static void OnRxWindow1TimerEvent( void )
{
    ...
    RxSlot = 0;
		log_info("trigger Rx Event on RxWin1\n");
...
{
    TimerStop( &RxWindowTimer2 );
    log_info("trigger Rx Event on RxWin2\n");
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    ...
    TimerStart( &MacStateCheckTimer );
		...
		// TxPowers = { 20, 16, 14, 12, 10, 7, 5, 2 }
		log_debug("fcntup:%d, sentCnt: %d, power:%d, dr:%d, Channel:%d\n",
				UpLinkCounter, sentCnt++, txPower, LoRaMacParams.ChannelsDatarate, channel);
